...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    ############################################################################
    hidden_input,cache_hidden = affine_relu_forward(X,self.params['W1'],self.params['b1'])
    scores,cache_output = affine_forward(hidden_input,self.params['W2'],self.params['b2'])
    ...
    #get the loss for the last layer and gradient with respect to output 
    loss,dout = softmax_loss(scores,y)
    reg_loss = 0.5 * self.reg * np.sum(self.params['W1'] * self.params['W1']) + 0.5 * self.reg * np.sum(self.params['W2'] * self.params['W2'])
    loss += reg_loss

    #start backward pass 
    #first back pass the output layer and hidden layer(as a variable)
    dhidden, grads['W2'],grads['b2'] = affine_backward(dout,cache_output)
   
    # back pass the first layer 
  ...
  def loss(self, X, y=None):
    ...
    for i in range(self.num_layers-1):
      ...
      if self.use_batchnorm :
        ...
        if self.use_dropout:
          out[i+1],cache4[i] = dropout_forward(out[i+1],self.dropout_param)
    #get paramters for the last layer 
    ...
    for i in range(t):
      if self.use_batchnorm:
        ...
        if self.use_dropout:
          ddrop[t-1-i] = dropout_backward(dout[t-i],cache4[t-1-i])
          dout[t-i] = ddrop[t-i-1]        
        dout[t-1-i] ,grads['W'+str(t-i)], grads['b'+str(t-i)] = affine_relu_backward(dout[t-i],cache3[t-i-1])
    #add regularization
    for i in range(self.num_layers):
