...
def affine_backward(dout, cache):
  ...
  x_shape = x.shape
  N = x_shape[0]
  D = np.product(x_shape[1:])
  dx = np.reshape(np.dot(dout,w.T),x_shape)
  X_temp = np.reshape(x,(N,D))
...
def dropout_forward(x, dropout_param):
  ...
  if mode == 'train':
    ...
    ###########################################################################
    #divide p to cast boolean variable to 0 and 1
...
#for each one of the input points
  for n in range(N):
  #for each one of the different filters
    for f in range(F):
      for  i in range(H_): 
        for j in range(W_):
...
def conv_backward_naive(dout, cache):
  ...
  for n in range(N) :   #for ith image
    for f in range(F) :   #for fth filter
      for height in range(H_):
        for width in range(W_):
          window = x_padded[n,:,height*stride:height*stride+HH,width*stride:width*stride+WW]
          dw[f] += window*dout[n,f,height,width]
          db[f] +=dout[n,f,height,width]
          dx_padded[n,:,height*stride:height*stride+HH,width*stride:width*stride+WW] += dout[n,f,height,width]*w[f]
    ...
    for c in range(C):
      for h in range(H_new):
        for w in range(W_new):
          window = x[n,c,h*s:h*s+HH,w*s:w*s+WW]
    ...
    for c in range(C):
      for h in range(H_):
        for w in range(W_):
          ...
          m = np.max(window)
          dx[n,c,h*stride:h*stride+pool_height,w*stride:w*stride+pool_width] = (window==m)*dout[n,c,h,w]
